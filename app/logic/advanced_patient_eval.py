...
def calcular_tfg_ckd_epi(creatinina, edad, sexo, raza="no_negro"):
    """
    Calcula la TFG utilizando la fórmula CKD-EPI (2009).
    
    Args:
        creatinina (float): Creatinina sérica en mg/dL
        edad (int): Edad en años
        sexo (str): 'M' para masculino, 'F' para femenino
        raza (str): 'negro' o 'no_negro'
        
    Returns:
        float: TFG calculada en ml/min/1.73m²
    """
    # Validar parámetros de entrada
    if creatinina <= 0:
        creatinina = 0.7 if sexo == 'F' else 0.9
    
    if edad <= 0:
        return 0.0
    
    # Factor de raza (no utilizado en la nueva versión de la fórmula)
    factor_raza = 1.159 if raza == "negro" else 1.0
    
    # Cálculos según sexo
    if sexo == 'F':  # Mujer
        if creatinina <= 0.7:
            tfg = 144 * pow(creatinina/0.7, -0.329) * pow(0.993, edad) * factor_raza
        else:
            tfg = 144 * pow(creatinina/0.7, -1.209) * pow(0.993, edad) * factor_raza
    else:  # Hombre
        if creatinina <= 0.9:
            tfg = 141 * pow(creatinina/0.9, -0.411) * pow(0.993, edad) * factor_raza
        else:
            tfg = 141 * pow(creatinina/0.9, -1.209) * pow(0.993, edad) * factor_raza
...
def generar_payload_gemini(paciente, laboratorios):
    ...
    # Calcular TFG
    tfg = calcular_tfg_ckd_epi(creatinina, edad, sexo, raza)
